...
# Add the repository root for direct-script compatibility.
ROOT_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
if ROOT_DIR not in sys.path:
    sys.path.insert(0, ROOT_DIR)

# Import the original OCR evaluation utilities
from lingt2i.data import DEFAULT_DATASET, TEXT_RENDERING_SPLIT, load_text_rendering_data
from .ocr_support.recognizer import TextRecognizer, crop_image
from .ocr_support.gemini_ocr import GeminiOCRRecognizer, crop_image_for_gemini
...
def setup_ocr_recognizer(language='en', use_gemini=False):
    ...
    rec_char_dict_path = os.path.join(os.path.dirname(__file__), 'ocr_support', 'ocr_recog', dict_file)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate text recognition models on LingT2I')
    parser.add_argument('--model_path', type=str, 
                        default=os.path.join(ROOT_DIR, "outputs", "text_rendering", "EasyText"),
...
def main():
    ...
    print("🚀 Starting LingT2I Text Recognition Evaluation")
    ...
    # Load the LingT2I text-rendering split.
