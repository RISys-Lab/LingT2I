...
DEFAULT_JUDGE_MODEL = "Qwen/Qwen2.5-VL-72B-Instruct-AWQ"
...
# ===== 异步主函数 =====
async def main_async(json_file, image_dir, model_name, batch_size=50, max_concurrent=5, model=DEFAULT_JUDGE_MODEL, endpoint="http://localhost:8000/v1/"):
    results = []
...
async def save_results(results, model_name):
    """异步保存结果"""
    # 保存详细结果
    result_dir = PROJECT_ROOT / "results" / "demographic_bias"
    result_dir.mkdir(parents=True, exist_ok=True)
    detailed_csv = result_dir / f"{model_name}.csv"
    ...
    # 保存统计结果
    stats_csv = result_dir / f"{model_name}_stats.csv"
...
# ===== 同步主函数（保持兼容性）=====
def main(json_file, image_dir, model_name, batch_size=50, max_concurrent=5, model=DEFAULT_JUDGE_MODEL, endpoint="http://localhost:8000/v1/"):
    """同步版本的主函数，内部调用异步版本"""
...
if __name__ == "__main__":
    ...
    for model_name in ["sd35", "sdxl", "X2I"]:
        main(
            json_file=str(PROJECT_ROOT / "data" / "legacy" / "text-to-image-multilingual.json"),
            image_dir=str(PROJECT_ROOT / "outputs" / "content_generation" / model_name),
